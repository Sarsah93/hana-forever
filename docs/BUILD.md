...
| 하고 싶은 것 | 방법 |
| --- | --- |
| 옮기기 | 하나 아래쪽 `하나` 손잡이를 드래그 |
| 행동 패널 열기/닫기 | **F2**, 또는 트레이 아이콘 우클릭 |
| 화면 중앙으로 데려오기 | 트레이 아이콘 우클릭 |
| **종료** | 트레이 아이콘 우클릭 → `하나 보내주기`, 또는 패널의 `종료` 버튼 |

행동 패널과 디버그 라벨은 배포 빌드에서 기본으로 숨겨집니다 (`npm run tauri dev`에서는 열린 채 시작).

## 다른 강아지로 바꾸기

`tools/mascot-kit/README.md` 를 참고하세요. 사진 폴더와 `mascot.config.json` 만
준비하면 스프라이트 생성부터 이름 치환까지 파이프라인이 처리합니다.
