...
같은 개로 보이도록 **머리 크기**를 먼저 맞추고, 그다음 자세별 높이가 그럴듯한지 본다. 실측(불투명 bbox 위 32% 구간의 최대 폭): 정면 서기 계열(대기·털기·웅크리기 서기·두 발 서기·점프) 머리 폭 88–94px로 일치. 측면 자세는 옆얼굴 ≈ 72px(기대기·엎드리기·뒤돌아보기)에 맞춰 걷기 .57(이전 .49는 머리가 정면 대기의 75%), 앉기·턱 긁기 .48(이전 .455), 하품·웃기 .45(이전 .41). 높이는 네발 서기 ≈ 155px, 걷기 ≈ 137px, 앉기 ≈ 155px, 두 발 서기 ≈ 215px, 엎드리기 ≈ 98px, 정면 엎드림 ≈ 130px. 결과는 `docs/previews/size-calibration.png`. motion-v2와 v3의 화풍 차이(아기/성견 인상)는 스케일로는 안 없어지며 런타임 디졸브로 가려진다.

## 전환에 쓰이는 프레임

- 정착 인트로(crouch-lick 0→1, lie-front 4→5)는 떠날 때 거꾸로 재생되어 일어나는 장면이 된다(`outroOf`). 새 시트를 만들 때 "서기→자세" 인트로 프레임을 넣어 두면 그대로 아웃트로가 된다.
- 프레임 차이가 눈·호흡·혀 정도인 시트는 `smooth`로 프레임 사이를 디졸브한다. 걷기·털기·긁기처럼 동작이 큰 시트는 컷을 유지한다.
